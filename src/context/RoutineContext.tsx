...
export const RoutineContext = createContext<[Routine[], (routine:Routine) =>void, AddExercise]
>([initialRoutineState, () => null, () => null])

export const RoutineContextProvider = ({children}: {children: React.ReactNode}): React.ReactElement => {
    ...
    const addRoutine = useCallback((routine:Routine) => {
        setRoutines(prevState => [...prevState, {...routine, id: uuidv4(), exercises:[] }])
    ...
    return <RoutineContext.Provider value= {[routines , addRoutine, addExercise]}>{children}</RoutineContext.Provider>;
}